...
Data da análise: 10/09/2026. Repositório: verticalpartsIA/010_GestaoImportacao. Referência consultada: main, árvore eb1fbc610825cb2b55eada80d32577aa2e537c0e.
...
Esta é uma análise do funcionamento previsto no código consultado. Uma primeira versão deste documento não conseguiu confirmar isso ao vivo (ver A01 no apêndice): o portal reconhecia o usuário, mas a abertura do card Cotação Importação | PRD ficava presa em carregamento. Em 10/09/2026 essa sessão foi reproduzida com sucesso — o aplicativo abre normalmente (com um redirecionamento breve pelo SSO do portal a cada carregamento direto de URL, sem travar) — e vários itens deste roteiro e do Apêndice A puderam ser conferidos clicando na tela real, não só lendo o código. Os itens efetivamente clicados estão marcados como confirmados nas seções correspondentes; a maior parte das 49 telas ainda depende só da leitura de código, listada no Apêndice B. Os arquivos de interface, roteamento e persistência foram consultados para construir o roteiro. As ressalvas e instruções de correção estão reunidas no apêndice.
...
Usaremos a fictícia Construtora Horizonte, obra Edifício Horizonte em São José dos Campos, interessada em dois elevadores: um de 630 kg e dez paradas, outro de 1.000 kg e dez paradas. Nenhum cadastro fictício foi inserido no sistema. Os valores e nomes do exemplo servem apenas para explicar a aula.
...
A categoria possui nove telas e um subtítulo organizador chamado Gestão Importação. O subtítulo não é uma tela. Confirmado ao vivo em 10/09/2026: o cabeçalho da própria categoria no menu também exibe um segundo rótulo menor, "Siscomex & Compras", abaixo do nome "Suprimentos & Importação" — isso é decorativo (não é uma tela nem um terceiro nível de menu). O grupo reúne Painel, P.I., RFQ, IMS, Embarques, Importação, Análise de Preços, Compras Nacional e Pedidos.
...
Confirmado ao vivo em 10/09/2026: a própria tela de Vistorias de Obras (Despachar) exibe o aviso "A execução no celular ainda é a próxima fase" — ou seja, o fluxo de despacho e o modelo de questionário já existem e funcionam, mas a experiência de execução pelo técnico no celular está sinalizada pelo próprio sistema como não concluída. Não apresentar esta etapa como pronta na aula sem essa ressalva.
...
As demonstrações com efeito externo devem usar registros de treinamento previamente definidos. Este levantamento não criou, aprovou, excluiu ou enviou registros operacionais.
...
## A01. [RESOLVIDO em 10/09/2026] Acesso pela sessão de navegação não chegou ao aplicativo

Evidência original: o portal exibiu o usuário autenticado e o card solicitado. A nova aba do aplicativo permaneceu em carregamento. A verificação em outra aba retornou ao portal em tela de login.

Verificação ao vivo (10/09/2026): a sequência portal → card Cotação Importação | PRD → aplicativo funcionou normalmente na sessão de teste. Cada carregamento direto de uma URL interna (ex.: digitar a rota e dar Enter) passa por um redirecionamento breve pelo SSO do portal (`portal.vpsistema.com`, tela "Bem-vindo ao portal… Carregando…") antes de voltar ao aplicativo — isso não é o travamento indefinido relatado antes, é um comportamento normal de poucos segundos. Não foi possível determinar se o travamento original era um problema pontual da sessão anterior ou algo já corrigido; não reproduziu nesta sessão. Dar aula ao vivo com o aplicativo está liberado.

## A02. [CORRIGIDO em 10/09/2026] Dashboard: seletor de período não filtra a carga de dados

Evidência original: period alterna entre Hoje/7/30/90, mas a carga usa loadDashboardData(role) e os dados renderizados não são filtrados pelo período. Fonte: src/dashboard.jsx.

Verificação ao vivo: clicando no botão de período (Hoje → 7 dias) no Dashboard Admin real, o rótulo mudou mas todos os KPIs (Projetos ativos, Embarques em trânsito, Alertas críticos, Faturamento) permaneceram idênticos. Bug real, confirmado.

Decisão de negócio confirmada: filtrar cada KPI pela data de criação/ocorrência do registro correspondente. Correção aplicada: "Projetos ativos" agora filtra por `start_date`, "Faturamento (propostas assinadas)" por `aprovada_em`, "Comissões (custo)" por `created_at` — os três repassados como recortes opcionais (`projetosPeriodo`/`propostasPeriodo`/`comissoesPeriodo`) para `AdminMetrics.kpis()`/`compute()`, 100% compatível com os testes existentes. "Embarques em trânsito" (foto do status atual) e "Alertas críticos" (mistura achados de consistência entre módulos sem data, ex. proposta aprovada sem contrato) continuam sem filtro de período de propósito — não fazem sentido "no período" sem inventar semântica nova. Testado: 11/11 testes unitários (8 existentes + 3 novos) e verificado ao vivo contra o Supabase real — Projetos ativos foi de 0 (Hoje) a 2 (7 dias) a 11 (30 dias); Faturamento de R$ 0 a R$ 470k.

## A03. [CORRIGIDO em 10/09/2026] Dashboard: checkbox da tarefa sem persistência

Evidência original: a linha de tarefa renderiza input checkbox sem checked/onChange ligados ao registro. Fonte: src/dashboard.jsx.

Correção aplicada: `loadDashboardData` (src/supabase.js) passou a incluir o `id` de cada tarefa (antes descartado no mapeamento); o checkbox em src/dashboard.jsx agora grava `tarefas.done = true` no Supabase ao ser marcado e recarrega o Dashboard. Pode ensinar o check como conclusão de tarefa normalmente.

## A04. [Decisão de negócio tomada em 10/09/2026 — implementação em sessão dedicada] Notificações: preferências só guardadas no navegador
...
Decisão: o disparo real não será por e-mail — vai por **WhatsApp** dos gestores/responsáveis, reaproveitando a infraestrutura já existente na VerticalParts (não criar uma integração nova do zero). Consultado via issue GitHub #348 ("Pergunta ao Claude-VPS", resposta de outra instância de Claude rodando na VPS):

- **Provedor**: instância própria self-hosted da **Evolution API** (não é Twilio nem Meta Cloud API oficial) — já usada por Verti/Pós-Venda 360, Escamax e VP Click.
- **Credenciais**: vivem em `.env` na VPS ou `supabase secrets`, nunca em texto de repositório público — precisam ser passadas por canal privado antes de implementar.
- **Número**: já ativo em produção (linha da Verti), uso unidirecional de saída, sem se misturar com o roteador de atendimento.
- **Padrão recomendado a seguir**: o motor `vpclick-cobranca/notificacoes.py` — detecção determinística em SQL/Python (não IA) + Claude Haiku só para redigir o texto final (batch, JSON schema); respeitar horário comercial (seg-sex 07h-18h SP, feriados via lib `holidays`); dedup por estado (nunca reenviar o mesmo evento); mensagem sempre se identificando como o sistema.
- **Contatos**: tabela `internal_contacts` (telefone, nome, cargo, depto) existe, mas está no Supabase do **pv360** (`jkbklzlbhhfnamaeislb`), projeto diferente do VP Gestão (`jxtqwzmpgofwctqajewt`) — acesso cross-project. ~27-30 colaboradores com telefone confirmado; nomes divergentes tratados por dicionário de alias manual (nunca fuzzy-match automático).

Por que a implementação ficou para depois: exige troca de credenciais por canal seguro, acesso a um segundo projeto Supabase e construção de um serviço novo seguindo o padrão acima — escopo de sessão dedicada, não um ajuste dentro de uma auditoria.

## A05. [CORRIGIDO em 10/09/2026] Controles sem ação ligada

Evidências de código: Ver agora no aviso financeiro; Versões e Duplicar na calculadora legada; Email fornecedor no detalhe de Importação; Exportar relatório no mapa; Transportadora em Compras; menu de opções em Usuários. Os elementos apareciam sem manipulador de ação nesse componente.

Correção aplicada: "Ver agora" (Prazos & Pendências) agora rola a tela até a Cadeia de Gatilhos por Cotação — a única ação segura e não-ambígua entre as seis. Os outros cinco (Versões, Duplicar, Email fornecedor, Exportar relatório, Transportadora, menu "..." de Usuários) exigiriam inventar uma regra de negócio ainda não definida (ex.: de onde vem o e-mail do fornecedor, o que "duplicar" preserva) — foram desabilitados com tooltip "Em desenvolvimento" explicando o motivo, em vez de continuar parecendo funcionais sem fazer nada.

Fontes: src/financeiro.jsx, src/precificacao.jsx e src/logistica.jsx.

## A06. [Corrigido parcialmente em 10/09/2026 — upload real ainda em aberto] Importar minuta informa sucesso sem persistir arquivo

Evidência original: o manipulador abria seletor, lia nome do arquivo e mostrava mensagem de "importação" com selo de sucesso, sem upload/persistência naquele fluxo. Fonte: src/operacoes.jsx, JuridicoPage.

Correção aplicada: o botão parou de mentir — em vez do falso aviso de sucesso, "Importar minuta" agora aparece desabilitado com tooltip "Em desenvolvimento — upload de minuta ainda não persiste o arquivo". Falta ainda a funcionalidade real (upload para um bucket do Supabase Storage vinculado ao contrato) — isso é a instrução original que permanece pendente: salvar o arquivo e sua referência, tratar erro e permitir reencontro depois de recarregar.

## A07. [Rótulo corrigido em 10/09/2026 — upload real ainda em aberto] Anexar no projeto e foto na instalação: seleção não é upload

Evidência original: os controles citados em Engenharia e Instalação em Campo selecionavam arquivo e exibiam mensagem, sem enviar o conteúdo pelo manipulador mostrado. Fonte: src/operacoes.jsx.

Situação em 10/09/2026: "Anexar" (Engenharia) já era honesto — mostra aviso tipo "info" dizendo que o arquivo só foi selecionado e que o bucket ainda precisa ser configurado. "Foto" (Instalação em Campo) usava um aviso tipo "success" (verde, parece confirmação de envio) com o mesmo texto ambíguo — foi alinhado ao mesmo aviso honesto do "Anexar". O upload real (persistência no Supabase Storage) continua pendente nos dois; a instrução original permanece: implementar upload, vínculo ao projeto/equipe/obra, abertura e falha de envio.

## A08. [CORRIGIDO em 10/09/2026] Dossiê: atalho de pendência da Visão Geral incompleto

Evidência original: havia botão com alert('Adicionar pendência') em TabVisaoGeral; a aba Pendências possui fluxo próprio com ModalAdicionarPendencia. Fonte: src/dossier-obra.jsx.

Correção aplicada: o botão da Visão Geral agora chama `setModalOpen('adionar-pendencia')`, o mesmo estado que a aba Pendências já usava para abrir o `ModalAdicionarPendencia` real. Não há mais implementação paralela — os dois atalhos abrem o mesmo modal funcional.

## A09. [CORRIGIDO em 10/09/2026] Inbox e atalhos de rastreamento precisam revisão

Evidência original: botões Responder, Vincular, Encaminhar e Sugerir resposta em EmailInbox não possuíam ação no componente. O botão Abrir embarque no mapa voltava à listagem de Importação, sem abrir o registro selecionado. Fonte: src/logistica.jsx.

Correção aplicada: no Mapa Marítimo (Importação → Ver mapa), "Abrir embarque" agora leva o navio selecionado (`setSubsel`) e navega para "importacao-detail" — abre o registro certo, não mais a listagem genérica (ajuste também em src/app.jsx e src/print-app.jsx, que instanciam esse componente). Os seis botões do Inbox sem integração real (Responder ×2, Vincular, "..." , Encaminhar, Sugerir resposta AI) foram desabilitados com tooltip explicando que o inbox ainda não está integrado a um serviço de e-mail real, em vez de simular uma caixa de entrada funcional.

Atualização (outra sessão, commits `f878d6c`/`60dce75`/`9c10821` em 10-11/09): a leitura do Inbox de Importação/Compras passou a ser real via IMAP, e o Inbox foi consolidado num único ponto de entrada no módulo Geral. Os tooltips dos botões "Responder"/"Responder a todos" foram atualizados pra deixar claro que a leitura já é real — só o envio de resposta de dentro do site continua não implementado. Não houve conflito com a correção acima.

## A10. [CORRIGIDO em 10/09/2026] Três controles de instalação/pagamento exigem regra única

Evidência original: Cronograma usa instalacao_cronograma; o Dossiê possui cronograma/checklist próprio; Pagamentos a Instaladores usa parcelas contratuais. A análise não demonstrou sincronização integral entre os três.

Investigação mais funda revelou a arquitetura real: **Pagamentos a Instaladores** (`contrato_instalador_parcelas`) já libera parcela automaticamente por evento real (`eventos_fluxo` — INSTALACAO_INICIADA/METADE_EXECUCAO/CONCLUIDA), não é manual. O problema real era que o evento do meio (METADE_EXECUCAO) vinha do **Cronograma de Instalação** (checklist de template, contagem simples de itens, aba do Dossiê) em vez do **Acompanhamento de Obra** (diário com foto do Montador, progresso ponderado por peso) — que o usuário confirmou ser a fonte de status de campo de verdade. Ou seja: o dinheiro liberava com base num checklist de template, não no diário real usado pra acompanhar a obra.

Decisão de negócio confirmada: o Diário de Obra (Acompanhamento de Obra) passa a ser a fonte única desse evento. Correção aplicada: `acompanhamento-obra-store.js` (`registrarLancamento`) agora dispara INSTALACAO_METADE_EXECUCAO ao atingir ≥50% de progresso ponderado, idempotente via `eventos_fluxo` (mesmo padrão que o checklist usava); o disparo duplicado foi removido de `instalacao-checklist-store.js` (o checklist de template continua existindo como cronograma operacional, só não decide mais sozinho a liberação de pagamento). A tela "Cronograma" (`instalacao_cronograma`, liberação manual redundante) fica como está por decisão explícita do usuário — fora de escopo desta correção.

Testado: sintaxe válida e tela carrega sem erro. **Não testado ao vivo o disparo real do evento** (efeito colateral financeiro real — poderia liberar pagamento de um instalador de verdade); decisão do usuário foi mergear mesmo assim e acompanhar o próximo caso real de obra atingindo ~50% pelo diário.

Fontes: src/acompanhamento-obra-store.js, src/instalacao-checklist-store.js, src/contrato-instalador-parcelas-store.js, src/entrega.jsx (não alterado), src/dossier-obra.jsx.

## A11. [CORRIGIDO em 10/09/2026] Entrega Final: destino fixo e indicador de garantia
...
Decisão de negócio confirmada: Escamax é de fato a parceira de manutenção real da VerticalParts hoje — o destino fixo está correto e não precisa virar configurável. Correção aplicada: o KPI "Em garantia" agora usa a data real (`data_fim_garantia`, já gravada no handover) via novo helper `HandoverManutencao.estaEmGarantia()`, em vez de `entregues - transferidos`. Testado localmente contra o Supabase real.

## A12. [CORRIGIDO em 10/09/2026] Remover alocações pode não retirar visibilidade
...
Decisão de negócio confirmada: "Remover acesso" deveria bloquear de verdade. Correção aplicada sem migração de schema: `colaborador_alocacoes` (src/colaboradores-admin-store.js) agora grava um marcador reservado (`__acesso_restrito__`, que nunca bate com nenhum label real de menu) quando todas as alocações são removidas — pelo botão de lixeira ou desmarcando manualmente o último módulo. Conceder qualquer módulo real limpa o marcador de novo. `src/shell.jsx` não precisou de nenhuma mudança — sua lógica de filtro por array vazio/não-vazio já funcionava certo, só faltava o marcador pra diferenciar "nunca configurado" de "removido de propósito". A lista de colaboradores agora mostra um badge "🔒 Acesso bloqueado" em vez do texto interno do marcador. Testado localmente contra o Supabase real (removerTodasAlocacoes/alocar/desalocar confirmados via SQL direto).

## A13. [Verificado em 10/09/2026 — não é um bug] Configurações: referências não são parâmetros editáveis
...
Verificação no código: os itens de ConfigParams (Parâmetros Financeiros/Operacionais) já são linhas somente-leitura (`ParamRow`), sem nenhum campo de input — não fingem ser editáveis. A Matriz de Permissões já se rotula "modelo de referência" no próprio título. ConfigIntegrations já tem um comentário no código explicando a decisão deliberada de listar "Não configurado" em vez de simular um status "Ativo" falso. Não há ação de código pendente aqui — a tela já é honesta sobre o que é e o que não é.

## A14. [Corrigido parcialmente em 10/09/2026 — Dashboard ainda em aberto] Dashboard e decisões: falha de carga pode parecer ausência de pendência

Evidência original: o Dashboard possui timeout que coloca estruturas vazias; a Central de Decisões captura erro da listagem convertendo-o em lista vazia. Fontes: src/dashboard.jsx e src/decisoes.jsx.

Correção aplicada (Central de Decisões): `DecisoesPage` agora guarda um estado `erro` separado do vazio; quando a consulta falha, mostra "Não foi possível carregar suas decisões pendentes agora" com um botão "Tentar novamente", em vez de "Nenhuma decisão pendente". Confirmado ao vivo, sem erros de console.

Ainda em aberto: o timeout do Dashboard (src/dashboard.jsx) que zera as estruturas continua sem essa distinção — está fora do escopo desta correção pontual porque envolve múltiplas fontes de KPI e o próprio desenho de timeout do carregamento inicial, não um catch isolado como em decisoes.jsx.

## A15. [CORRIGIDO em 10/09/2026] Painel de Importação: total de P.I.s em moedas diferentes

Evidência original: o painel somava os valores das P.I.s abertas e formatava o total com a moeda da primeira P.I. da lista — misturando USD, BRL e EUR num único número. Fonte: src/gestao-importacao-painel.jsx.

Correção aplicada: o KPI "P.I. abertas" agora agrupa e soma os valores por moeda separadamente e mostra um total por moeda (ex.: "US$ 120.000,00 + € 4.000,00") em vez de somar tudo com um único símbolo. Não foi feita conversão cambial (exigiria escolher taxa e data, decisão de negócio) — a correção é o agrupamento, como a instrução original já sugeria como primeira opção.

## A16. [Atualizado em 10/09/2026] Pontos que ainda exigem inspeção em produção
...
Para concluir a homologação da aula, percorrer a lista do apêndice B em produção, abrir pelo menos um registro por tela, expandir cada aba e verificar os estados relevantes. Registrar a versão publicada e as diferenças encontradas. Em 10/09/2026 as 49 telas do índice do Apêndice B foram abertas e conferidas ao vivo (uma navegação por rota, checando título, conteúdo carregado e ausência de erro de console), além de testes mais profundos nos itens do Apêndice A (A01, A02, A03, A05, A08, A09, A13, A14, A15) com correções aplicadas onde havia bug real e simples de resolver. Isso não é uma homologação completa por aba/modal/estado/perfil — apenas confirma que todas as rotas carregam e a estrutura geral bate com este roteiro. Achados adicionais dessa passada: a P.I. de teste "teste01" tem uma data de abertura com ano digitado errado (6206) — dado de produção, não bug de código; e a tela de Vistorias de Obras confirma ao vivo que a execução por celular do técnico "ainda é a próxima fase" (ver nota em 13.4). Uma homologação mais profunda (abrir cada aba/modal, testar com múltiplos perfis, popular estados vazios com dados de teste) continua pendente.

## Retomar aqui — auditoria campo-a-campo (sessão de 10-11/09/2026)

O usuário pediu que a auditoria fosse além da checagem de rota (item acima) — abrir cada tela de verdade e comparar campo a campo, botão a botão, com este documento, corrigindo divergências reais no código, no `.md` e no `.xmind`. Já concluído nessa profundidade: **Precificação** (bate 100%; achado que "Calcular" grava no banco, não é preview — cuidado ao testar com cotação real), **Aval Financeiro**, **Contrato Venda de Equipamentos**, **Contrato Instalador**, **Pagamentos a Instaladores** (achado e corrigido: 2 textos citando o Cronograma de Instalação como fonte do gatilho de pagamento, desatualizados desde o A10 — agora citam Acompanhamento de Obra).

**Continuar por aqui, na mesma profundidade**: Comissões (4.4); P.I./RFQ/IMS/Embarques (6.2–6.5); Ficha Técnica (7.5) e Produtos (7.6); Dossiê de Obras — as 9 abas (8.1); Vistorias de Obras e Resultado Vistorias (8.2–8.3); Central de Documentos, Data Book, Entrega Final (9.1–9.3); Empresas Instaladoras e Homologação de Instaladores (10.1–10.2); Almoxarifado (11.1); Configurações do Sistema (12.2, as 6 abas). Ao terminar, atualizar este documento e o `.xmind` com o mesmo padrão usado nas correções acima (achado, correção aplicada, o que ficou pendente).

Cuidado ao testar: várias telas gravam de verdade ao clicar em ações que parecem só "calcular"/"visualizar" (ex.: Precificação → Calcular). Evite testar contra cotações/contratos/obras reais quando a ação grava; prefira ler o código-fonte pra confirmar comportamento, ou usar um registro de treinamento se precisar clicar de verdade.
...
O roteiro está entregue como base de código para a aula. A validação visual completa continua pendente: abrir cada tela do índice, cada detalhe/aba, testar filtros e arquivos, conferir herança com registros vinculados e demonstrar estados sem alterações acidentais. Não se deve rotular este material como um tour integralmente testado por cliques nesta sessão.
