...
Roteiro de aula: Tour VIII do VP Gestão.
...
Para o mapa mental, trate cada grupo abaixo como um mapa independente. “CRM” é um grupo visual sem tela disponível e deve ser marcado como “Em breve”. “GESTÃO IMPORTAÇÃO” é o subtítulo interno de SUPRIMENTOS & CADEIA DE SUPRIMENTOS, não uma tela. O rodapé mostra o usuário ativo, e o botão “Toggle sidebar” recolhe ou expande a navegação.

| Mapa | Ramos exibidos |
| --- | --- |
| Geral | Dashboard; Notificações; Central de Decisões; Preços & Pendências; Inbox |
| CRM | Em breve, sem submódulos navegáveis |
| PARCEIROS & MESTRES | Clientes; Fornecedores |
| COMERCIAL \| PRÉ VENDA | Leads; Formulários; Controle de Cotações; Cotações e Fornecedor; Propostas |
...
| SUPRIMENTOS & CADEIA DE SUPRIMENTOS → GESTÃO IMPORTAÇÃO | Painel; P.I.; RFP; IMS; Embarques; Importação; Análise de Preços; Compras Nacional; Pedidos |
| ENGENHARIA & PRODUTO | Engenharia; Projeto de Elevadores; Projeto de Equipamento; Projetos EVEs; Ficha Técnica; Produtos; Linha do Tempo da Cotação |
| OBRAS & INSTALAÇÃO | Dossie de Obras; Vistorias de Obras; Resultado Vistorias de Obras; Instalação em Campo; Cronograma; ART |
| ENTREGA & DOCUMENTAÇÃO | Central de Documentos; Guia Book & Termo; Entrega Final |
| PARCEIROS & INSTALAÇÃO | Empresas Instaladores; Homologação de Instaladores; Contrato Instalador; Pagamentos e Instaladores |
| LOGÍSTICA INTERNA | Almoxarifado |
| ADMINISTRAÇÃO | Logs de Atividade; Configurações do Sistema |

O nome “Dossie de Obras” é o rótulo da tela de obras usado neste mapa. “Guia Book & Termo” é o rótulo da tela de entrega documental. “RFP” é o título amigável usado na sidebar; a rota e alguns componentes técnicos ainda usam a sigla RFQ de importação. No rodapé observado ao vivo, o usuário exibido foi “Gelson Simoes”, não “Deilson Simões”; o roteiro mantém “Deilson Simões” como o título original solicitado para a aula.

# 1. GERAL

Esta categoria reúne cinco telas que ajudam a saber como está a operação, o que chegou, quem precisa decidir e qual prazo está pendente. Sua proposta é permitir que o usuário encontre a próxima ação antes de abrir cada departamento separadamente.
...
A Inbox concentra a caixa compartilhada da VerticalParts para leitura, resposta, encaminhamento e vínculo de e-mails com cotações. É uma tela operacional: o usuário lê a mensagem, identifica a origem e decide se ela deve alimentar uma etapa comercial, de compras ou de importação.

### Pastas e estados

| Pasta | Proposta e comportamento |
| --- | --- |
| Caixa de entrada | Consulta as mensagens recebidas pelo serviço de leitura da caixa compartilhada e mostra a contagem de não lidas. |
| Enviados | Consulta os envios gravados em `emails_projeto`, inclusive mensagens enviadas por RFQ, Responder ou Novo e-mail. |
| Rascunhos | Pasta exibida na interface, mas ainda sem leitura real de dados no código validado. |
| Arquivados | Pasta exibida na interface, mas ainda sem leitura real de dados no código validado. |

### Cabeçalho e Novo e-mail

Voltar retorna ao Dashboard. Atualizar recarrega a Caixa de entrada. Novo e-mail abre uma janela para informar destinatário, assunto, corpo, anexos e, opcionalmente, o Nº Cotação que já deve nascer vinculado ao projeto. A caixa usada é `suporte@vpsistema.com`, compartilhada por Comercial, Compras e Importação.

### Mensagem, origem e vínculo

Ao selecionar uma mensagem, a tela apresenta remetente, data, assunto, corpo e anexos. Quando existe Nº Cotação, o selo abre a Linha do Tempo da Cotação. Vincular permite informar manualmente um número de cotação; o sistema confirma se a cotação existe e grava o vínculo como certo em `emails_projeto`.

### Responder, responder a todos e encaminhar

Responder usa o remetente original. Responder a todos reúne remetente, destinatários e cópia, excluindo a própria caixa da VerticalParts. Encaminhar exige um novo destinatário e inclui a mensagem original. O campo de texto é obrigatório para enviar resposta; Anexar aceita múltiplos arquivos até o limite configurado. Sugerir resposta (AI) apenas preenche uma sugestão para revisão humana, não envia automaticamente.

### Resposta do fornecedor e gatilho

Quando a mensagem está ligada a uma cotação que possui o gatilho “Aguardando resposta do Fornecedor” aberto, a Inbox oferece Marcar como resposta do fornecedor. O botão registra `FORNECEDOR_RESPONDEU` e fecha o gatilho. Ler ou receber um e-mail não deve ser confundido com aprovar uma compra.

### Herança e destino

A Caixa de entrada herda mensagens do serviço `read-inbox`; os envios são lidos de `emails_projeto` com `direcao = saida`. Respostas podem herdar o Nº Cotação da mensagem original e gravam a referência `resposta_inbox`. O vínculo alimenta a Linha do Tempo da Cotação e pode apoiar o fechamento do gatilho de resposta do fornecedor.

Fonte: `src/logistica.jsx`, componente `EmailInbox`, e funções `read-inbox`, `send-email` e `suggest-email-reply`.
...
Cinco telas conduzem a oportunidade até a oferta: Leads, Formulários, Controle de Cotações, Cotações e Fornecedor e Propostas. A proposta desta categoria é conservar a identidade do cliente e dos equipamentos durante a negociação.
...
Recebe Proposta, cotação e cliente. Registra consulta, decisão, sinal e aprovações. Alimenta os gates usados em Cotações e Fornecedor e P.I. e registra eventos para acompanhamento. No exemplo, a aprovação comercial da Horizonte ainda passa por esta análise antes de comprometer a compra.
...
# 6 SUPRIMENTOS & CADEIA DE SUPRIMENTOS

# 7 GESTÃO IMPORTAÇÃO

A categoria 6 representa a área de Suprimentos & Cadeia de Suprimentos. O módulo 7, Gestão Importação, reúne nove telas: Painel, P.I., RFP, IMS, Embarques, Importação, Análise de Preços, Compras Nacional e Pedidos.
...
Ver mapa abre mapa. Atualizar solicita sincronização do rastreamento. Mostrar todos os eventos expande a lista. Ver no Financeiro abre Preços & Pendências. Reportar/confirmar chegada registra a chegada e cria tarefas previstas para os setores. Esse comando tem efeito de negócio.
...
# 8 ENGENHARIA & PRODUTO

Sete telas definem e documentam o produto: Engenharia, Projeto de Elevadores, Projeto de Equipamento, Projetos EVEs, Ficha Técnica, Produtos e Linha do Tempo da Cotação. A proposta é conservar a especificação técnica e a identificação do equipamento durante venda, compra, obra e entrega.
...
Seis telas organizam execução: Dossie de Obras, Vistorias de Obras, Resultado Vistorias de Obras, Instalação em Campo, Cronograma e ART. O Dossiê é a principal página mãe desta categoria, porque reúne os registros da obra e suas relações.
...
## 9.3 Resultado Vistorias de Obras
...
# 11 PARCEIROS & INSTALAÇÃO

Quatro telas organizam o ciclo do parceiro: Empresas Instaladores, Homologação de Instaladores, Contrato Instalador e Pagamentos e Instaladores. A proposta é ligar a pessoa jurídica, sua equipe, documentos, ativos contratados, execução e pagamentos sem confundir os diferentes níveis.

## 11.1 Empresas Instaladores
...
## 11.2 Homologação de Instaladores
...
Recebe proposta/ativos, obras e dados do parceiro. Alimenta o vínculo contratual da instalação e a base de parcelas consultada em Pagamentos e Instaladores. A assinatura é um evento separado da criação do documento.
...
## 11.4 Pagamentos e Instaladores
...
# 14 Rodapé da Sidebar

O rodapé identifica o usuário ativo e concentra o acesso final da navegação. Para a aula e para o mapa mental, o rótulo original informado é o nome abaixo.

## 14.1 Usuário Logado

Usuário exibido no rodapé da Sidebar. Este ramo é de identificação do perfil, não uma tela operacional.

# 15. Telas externas e ramos transversais
...
## 15.1 Formulário público do cliente
...
## 15.2 Resposta pública do fornecedor
...
## 15.3 Proposta e assinatura pública
...
# 16. Sequência sugerida para a aula
...
# Registro de validação ao vivo do Tour VIII
...
4. Geral → Central de Decisões, Preços & Pendências e Inbox, incluindo composição, resposta, encaminhamento e vínculo de e-mail.
5. Cadastros Mestres → Clientes e Fornecedores, com abertura de cadastro, busca, edição/histórico e categorias.
6. Comercial → Lead, detalhe, Formulário, Controle de Cotações, Cotações e Fornecedor, detalhe/resposta/tratativas e Propostas/editor.
...
Evidência: Cronograma usa instalacao_cronograma; o Dossiê possui cronograma/checklist próprio; Pagamentos e Instaladores usa parcelas contratuais. A análise não demonstrou sincronização integral entre os três.
...
| Geral | Preços & Pendências | `/adm-financeiro/financeiro` |
...
| Comercial / Pré-venda | Cotações e Fornecedor | `/adm-financeiro/cotacoes-fornecedor` |
...
| Engenharia & Produto | Projetos EVEs | `/engenharia/desenho-tecnico` |
...
| Obras & Instalação | Dossie de Obras | `/engenharia/status-obras` |
...
| Entrega & Documentação | Guia Book & Termo | `/engenharia/databook` |
| Entrega & Documentação | Entrega Final | `/engenharia/handover` |
| Parceiros & Instaladores | Empresas Instaladores | `/cadastros/cadastro-instaladores` |
...
| Parceiros & Instaladores | Pagamentos e Instaladores | `/adm-financeiro/pagamentos-instalador` |
